...
$tipo=$_POST['tipo'];
	...
	$query = "SELECT * FROM usuario WHERE nombre_usuario='$usuario'and id_rol=1";
        ...
        alert("Los datos insertados son incorrectos, pruebe de nuevo");
        </script>';
        ?> <script>
    ...
    <?php
	 ...
	 elseif($tipo=='Vendedor'){
		  ...
		  $query= "select usuario.id_usuario, usuario.nombre_usuario, usuario.contrasenia, usuario.id_sucursal_f, sucursal.nombre_sucursal, sucursal.id_sucursal from usuario 
...
$result = $conn->query($query);
    if($result->num_rows>0){
	 while($row=$result->fetch_assoc()) {
    if (password_verify($pass, $row['contrasenia'])) {
     $_SESSION['vendedor']=$row['nombre_usuario']; 
